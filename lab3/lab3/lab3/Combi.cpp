...
namespace combi
{
	subset::subset(short n) // ÃÅÍÅÐÀÒÎÐ ÌÍÎÆÅÑÒÂÀ ÂÑÅÕ ÏÎÄÌÍÎÆÅÑÒÂ
	{
		this->n = n;
	...
	xcombination::xcombination(short n, short m) // ÃÅÍÀÐÀÒÎÐ ÑÎ×ÅÒÀÍÈÉ
	{
		this->n = n;
		this->m = m;
	...
	void  xcombination::reset()     // ñáðîñèòü ãåíåðàòîð, íà÷àòü ñíà÷àëà 
	{
		this->nc = 0;
	...
	short xcombination::getnext()    // ñôîðìèðîâàòü ñëåäóþùèé ìàññèâ èíäåêñîâ  
	{
		short rc = getfirst();
		if (rc > 0)
		{
	...
	permutation::permutation(short n) // ÃÅÍÅÐÀÒÎÐ ÏÅÐÅÑÒÀÍÎÂÎÊ
	{
		this->n = n;
		this->sset = new short[n];
	...
	accomodation::accomodation(short n, short m) // ÃÅÍÅÐÀÒÎÐ ÐÀÇÌÅÙÅÍÈÉ
	{
		this->n = n;
		this->m = m;
